...
function Token() {
  ...
  const sub =
    currentSub && Array.isArray(currentSub)
      ? currentSub.map((element) => element?.sub || "N/A")
      : [];
  ...
  if (time >= 9.15 && time < 10.15) onTimeSub = sub[0] || "No Classes Found";
    ...
    onTimeSub = sub[1] || "No Classes Found";
  else if (time >= 11.15 && time < 12.15)
    onTimeSub = sub[2] || "No Classes Found";
  ...
  return (
    <div className='token_contain'>
      ...
      <form
        className={`token_box ${isChecking ? "checking" : ""}`}
      ...
      >
        ...
        <input
          {...register("subToken", {
            ...
            pattern: {
              value: /^[A-Z0-9]{4}$/,
              message: "Token must be 4 uppercase letters/numbers",
            },
          })}
        ...
        />
        {errors.subToken && (
          <span className='error'>{errors.subToken.message}</span>
        )}
        <button className='submit_btn' type='submit' disabled={isChecking}>
